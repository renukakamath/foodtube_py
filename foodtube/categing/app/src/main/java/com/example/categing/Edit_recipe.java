...
public class Edit_recipe extends AppCompatActivity implements JsonResponse{
    ...
    String name,dtls,inc,dietmode,Calorie,recipy_id;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        JR.json_response = (JsonResponse) Edit_recipe.this;
        String q = "/View_recipe?rid=" + MYProfile.vid;
        q = q.replace(" ", "%20");
        JR.execute(q);
...
//        	longitude=LocationService.logi;
            ...
            String q = "http://" + IPSettings.text + "/api/Edit_recipe";
...
//	        	String q= "api.php?action=teacher_upload_notes&sh_id="+Teacher_view_timetable.s_id+"&desc="+des;
            ...
            aa.put("rid",MYProfile.vid.getBytes());
...
//            aa.put("password",pass.getBytes());
            ...
            fua.json_response = (JsonResponse) Edit_recipe.this;
            fua.execute(aa);
        } catch (Exception e) {
...
//        final CharSequence[] items = {"Capture Photo", "Choose from Gallery" ,"Choose From Video", "Cancel"};
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(Edit_recipe.this);
...
//	                image = decodeFile(selectedImagePath);
    ...
    public void response(JSONObject jo) {
        ...
        try {
            ...
            if(method.equalsIgnoreCase("View_recipe")){
                String status=jo.getString("status");
//                Toast.makeText(getApplicationContext(), "name"+status, Toast.LENGTH_SHORT).show();
                Log.d("pearl",status);
//                if(status.equalsIgnoreCase("successs")){
//
//                    e1.setText(jo.getString("name"));
//                    e2.setText(jo.getString("incredints"));
//                    e3.setText(jo.getString("additional_dtls"));
//                    e4.setText(jo.getString ("calorie"));
//                    Toast.makeText(getApplicationContext(), "name"+e1, Toast.LENGTH_SHORT).show();
//
//
//
//
...
//                    Toast.makeText(getApplicationContext(), "hi" + ja1.getJSONObject(0).getString("firstname"), Toast.LENGTH_LONG).show();
                    ...
                    e2.setText(ja1.getJSONObject(0).getString("incredints"));
//                    e7.setText(ja1.getJSONObject(0).getString("place"));
                    e3.setText(ja1.getJSONObject(0).getString("additional_dtls"));
                    e4.setText(ja1.getJSONObject(0).getString("calorie"));

                    recipy_id = ja1.getJSONObject(0).getString("recipy_id");
